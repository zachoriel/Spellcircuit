...
//#pragma multi_compile _BLENDMODE_BLEND _BLENDMODE_MUL
...
#define _Cutout_arr Props
	...
	half4 frag(v2f i) : SV_Target
	{
		...
		half3 normal = abs(normalize(cross(ddx(wpos), ddy(wpos) * _ProjectionParams.x)));
		half height = normal.y;
		//normal = pow(normal, 10);
		float2 uv = wpos.yz * normal.x * 0.1 + wpos.xz * 1 + wpos.xy * normal.z * 0.1;
		...
		tintColor.rgb = tintColor.rgb * tintColor.rgb * 10;
		...
		half mask = tex.a;
	#endif
		half cutout = 0.5 + UNITY_ACCESS_INSTANCED_PROP(_Cutout_arr, _Cutout) * i.color.a * 0.5;
